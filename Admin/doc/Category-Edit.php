...
include 'Firstlogin.php';
  ...
  if($_POST)
  {
    ...
    $q=mysqli_query($connection,"UPDATE `tbl_category` SET `category_name` ='{$category_name}' where category_id='{$cat_id}'");
  }
?>
<!DOCTYPE html>
<html lang="en">
			...
			if($_POST)
				{
                  ...
                  <button class='btn-close' type='button' data-bs-dismiss='alert'></button> Record Inserted.  
                    </div><script>window.location='Category-Table.php'</script>";
				}
		?>
          <div class="tile">
            ...
            <div class="tile-body">
              <form class="form-horizontal" id="categoryform" method="post">
               ...
               <?php 
               $q=mysqli_query($connection,"SELECT * FROM tbl_category WHERE category_id='{$cat_id}'");
               $qdata=mysqli_fetch_array($q);
               ?>
                <div class="mb-3 row">
                  <label class="form-label col-md-3">Category name</label>
                  <div class="col-md-8">
                    <input class="form-control col-md-8" type="text" placeholder="Enter name of category" name="txt2" value="<?php echo $qdata['category_name']?>"required>
                  </div>
                </div>
