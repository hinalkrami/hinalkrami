...
<?php
 ...
 $sid=$_GET['eid'];
  if($_POST)
  {
    ...
    $q=mysqli_query($connection,"UPDATE  `tbl_subcategory` SET `category_id`='{$category_id}' , `subc_name`='{$subcategory_name}' WHERE subc_id='{$sid}'");
  }
?>
<!DOCTYPE html>
<html lang="en">
			...
			if($_POST)
				{
                  ...
                  <button class='btn-close' type='button' data-bs-dismiss='alert'></button> Record Updated.  
                    </div><script>window.location='Subcategory-Table.php';</script>";
				}
		?>
          <div class="tile">
            ...
            <div class="tile-body">
              ...
              <div class="mb-3 row">
                ...
                <div class="mb-3 row">
                  ...
                  <div class="col-md-8">
                    <?php
                    $sq=mysqli_query($connection,"SELECT * FROM tbl_subcategory WHERE subc_id={$sid}");
                    $data=mysqli_fetch_array($sq);
                    ?>
                    <input class="form-control col-md-8" type="text" placeholder="Enter name of Subcategory" value="<?php echo $data['subc_name']?>" name="txt2" required>
                  </div>
                </div>
